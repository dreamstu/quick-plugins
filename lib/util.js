var register = function(quick){
    ...
    _.findConf = function(done){
        return new _.Promise(function(resolve, reject) {
            var fup = _.findup(process.cwd(), quick.config.confFileName);
            var dir = null;
            ...
            fup.on('end', function() {
                ...
                if(dir!=null){
                    ...
                    quick.log.error('请检查{{name}}配置文件是否存在！'.replace('{{name}}',config.confFileName));
                }
            });
        }).then(function(dir){
                done(dir);
                ...
                quick.log.error('查找{{name}}配置文件失败！'.replace('{{name}}',config.confFileName));
            });
    };
